  ...
  return (
    <Box sx={{ display: "flex", alignItems: "flex-start", mb: 3 }}>
      {PIPELINE_ROUNDS.map((step, idx) => {
        ...
        const isCompleted =
          !isRejected && (dec === "selected" || step.round < activeRound);
        ...
        return (
          <Box
            key={step.label}
            sx={{ display: "flex", alignItems: "center", flex: isLast ? 0 : 1 }}
          >
            <Box
              sx={{
                display: "flex",
            ...
            {!isLast && (
              <Box
                sx={{
                  flex: 1,
                  height: 2,
                  bgcolor: isCompleted && !isRejected ? "#10B981" : "#E5E7EB",
                  mx: 0.5,
                  mb: 2.5,
...
export function ScheduleInterviewModal({
  ...
  const [decision, setDecision] = useState<"selected" | "rejected" | null>(
    null,
  );
  const [intimationNote, setIntimationNote] = useState(
    defaultNote(candidate.name, candidate.role),
  );
  ...
  useEffect(() => {
    availableInterviewersRef.current = availableInterviewers;
  }, [availableInterviewers]);
    ...
    try {
      ...
      if (activeRound) {
        ...
        if (activeRound.interviewers?.length) {
          const restored: Interviewer[] = activeRound.interviewers.map(
            (name, i) => {
              const COLORS = [
                "#6366F1",
                "#0EA5E9",
  ...
  function addInterviewer() {
    ...
    if (!name) return;
    if (
      interviewerList.some((iv) => iv.name.toLowerCase() === name.toLowerCase())
    ) {
      setNewInterviewerName("");
      return;
    }
    ...
    const initials = name
      ...
      .toUpperCase()
      .slice(0, 2);
    const newIv: Interviewer = {
      id: `iv-${Date.now()}`,
      name,
      initials,
      color: COLORS[interviewerList.length % COLORS.length],
    };
  ...
  const isMeetingLinkValid = form.meetingLink.trim().length > 0;

  const hasInterviewers = selectedInterviewers.length > 0;

  const canSendInvite =
    !!form.date && !!form.time && isMeetingLinkValid && hasInterviewers;
  ...
  async function handleSendInvite() {
    if (!form.date || !form.time) {
      return setSnackbar({
        open: true,
        message: "Please fill in date and time.",
        severity: "error",
      });
    }
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message: `Invite sent to ${candidate.name} ✓`,
        severity: "success",
      });
  ...
  async function handleSubmitDecision() {
    if (!decision) {
      return setSnackbar({
        open: true,
        message: "Please select a decision first.",
        severity: "error",
      });
    }
  ...
  const alreadyDecided =
    activeRoundData?.decision !== "pending" &&
    activeRoundData?.decision !== undefined;
        ...
        >
          ...
          >
            <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
              ...
              <Box>
                <Typography
                  sx={{ fontWeight: 700, fontSize: 15, color: "#111827" }}
                >
                  Schedule interview
                </Typography>
                <Typography sx={{ fontSize: 12, color: "#9CA3AF" }}>
                  Book a session with the candidate
                </Typography>
              </Box>
            </Box>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
              ...
              {(["schedule", "decision"] as const).map((tab) => (
                <Box
                  ...
                  sx={{
                    ...
                    boxShadow:
                      section === tab ? "0 1px 4px rgba(0,0,0,0.08)" : "none",
                ...
                >
                  <Typography
                    sx={{
                      ...
                      color: section === tab ? "#111827" : "#6B7280",
                    }}
                  >
                    {tab === "schedule"
                      ? "📅 Interview Details"
                  ...
                  <>
                    ...
                    <Box sx={{ mb: 2 }}>
                      ...
                      <Select
                        ...
                        onChange={(e) =>
                          setField("interviewType", e.target.value)
                        }
                        sx={{
                          borderRadius: "8px",
                          fontSize: 13,
                    ...
                    <Box sx={{ mb: 2 }}>
                      ...
                      <TextField
                        fullWidth
                        size="small"
                        placeholder="e.g. .NET / SQL"
                        value={form.focusArea}
                        onChange={(e) => setField("focusArea", e.target.value)}
                        sx={{
                          "& .MuiOutlinedInput-root": {
                            borderRadius: "8px",
                            fontSize: 13,
                    ...
                    <Box sx={{ mb: 2 }}>
                      ...
                      <TextField
                        ...
                        value={form.meetingLink}
                        onChange={(e) =>
                          setField("meetingLink", e.target.value)
                        }
                        InputProps={{
                          startAdornment: (
                            <LinkIcon
                    ...
                    <Box sx={{ mb: 2 }}>
                      <FieldLabel>
                        Interviewers <span style={{ color: "#EF4444" }}>*</span>
                      </FieldLabel>
                          ...
                          {interviewerList.map((iv) => (
                            <Box
                              ...
                              sx={{
                                ...
                                borderRadius: "20px",
                                border: "1.5px solid",
                                borderColor: selectedInterviewers.includes(
                                  iv.id,
                                )
                                  ? iv.color
                                  : "#E5E7EB",
                                bgcolor: selectedInterviewers.includes(iv.id)
                                  ? `${iv.color}14`
                                  : "#F9FAFB",
                                cursor: "pointer",
                                transition: "all 0.15s",
                                userSelect: "none",
                                "&:hover": { borderColor: iv.color },
                              }}
                            ...
                            >
                              ...
                              <Box
                                onClick={(e) => {
                                  e.stopPropagation();
                                  setInterviewerList((p) =>
                                    p.filter((x) => x.id !== iv.id),
                                  );
                      ...
                      <Box sx={{ display: "flex", gap: 1 }}>
                        <TextField
                          size="small"
                          fullWidth
                          placeholder="Type interviewer name and press Add"
                    ...
                    <Box sx={{ mb: 3 }}>
                      ...
                      <TextField
                        ...
                        rows={2}
                        size="small"
                        placeholder="e.g. Please have your IDE ready, focus areas are .NET and SQL…"
                        value={form.notes}
                        ...
                        >
                          <Box
                            ...
                            sx={{
                              ...
                              border: "2px solid",
                              borderColor:
                                decision === "selected" ? "#10B981" : "#E5E7EB",
                              bgcolor:
                                decision === "selected" ? "#F0FDF4" : "#FAFAFA",
                              cursor: "pointer",
                              transition: "all 0.15s",
                              display: "flex",
                              alignItems: "center",
                          ...
                          <Box
                            ...
                            sx={{
                              ...
                              border: "2px solid",
                              borderColor:
                                decision === "rejected" ? "#EF4444" : "#E5E7EB",
                              bgcolor:
                                decision === "rejected" ? "#FEF2F2" : "#FAFAFA",
                              cursor: "pointer",
                              transition: "all 0.15s",
                              display: "flex",
                              alignItems: "center",
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              ...
                              >
                                Default note — will be sent automatically
                              </Typography>
                            </Box>
                            <Button
                              size="small"
                          ...
                          {editingNote ? (
                            <TextField
                              ...
                              multiline
                              rows={6}
                              value={intimationNote}
                              onChange={(e) =>
                                setIntimationNote(e.target.value)
                              }
                              sx={{
                                "& .MuiOutlinedInput-root": {
                                  borderRadius: 0,
            ...
            {section === "schedule" ? (
              <>
                ...
                <Button
                  ...
                  disabled={sendingInvite || loading || !canSendInvite}
                ...
                {!alreadyDecided && (
                  <Button
                    ...
                    sx={{
                      textTransform: "none",
                      borderRadius: "9px",
                      bgcolor: decision === "rejected" ? "#EF4444" : "#2563EB",
                      fontWeight: 600,
                      fontSize: 13,
                      px: 2.5,
                      boxShadow: "none",
                      "&:hover": {
                        bgcolor:
                          decision === "rejected" ? "#DC2626" : "#1D4ED8",
                        boxShadow: "none",
                      },
                    }}
                  >
